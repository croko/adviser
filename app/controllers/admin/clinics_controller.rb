class Admin::ClinicsController < Admin::BaseController
  ...
  # GET /clinics.json
  def index
    @q = Clinic.includes(:manager).search(params[:q])
  ...
  def new
    if params[:advise_id].present?
      ...
      if advise.comment.present?
        @clinic.comments.build(message: advise.comment,
  ...
  def create
    ...
    if @clinic.advise.present?
      @clinic.advise.update_column('processed', true)
    end
  ...
  def clinic_params
    params.require(:clinic).permit(:id, :full_name, :description, :user_id, :published,
                                   :starting_work, :awards, :advise_id, :end_promo,
